...
export default async function ConsultantProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  const project = await prisma.project.findUnique({
    ...
    include: {
      ...
      assignees: {
        include: { learner: { select: { id: true, name: true, email: true, avatar: true } } },
      },
      versions: {
        orderBy: { versionNumber: "desc" },
        include: { signoffs: { include: { user: { select: { id: true, name: true, role: true } } } } },
      },
  ...
  const allVersions = [...project.versions].sort((a, b) => b.versionNumber - a.versionNumber);
  const activeVersion = project.versions.find((v) => v.isActive);
  const pendingVersion = project.versions.find((v) => v.status === "PENDING");

  // Compute diff between pending and active version (if pending exists)
  const diff =
    pendingVersion && activeVersion
      ? diffProjectVersions(
        {
          descriptionText: activeVersion.descriptionText,
          descriptionJson: activeVersion.descriptionJson as Record<string, unknown>,
        },
        {
          descriptionText: pendingVersion.descriptionText,
          descriptionJson: pendingVersion.descriptionJson as Record<string, unknown>,
        },
        {
          title: project.title,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-6xl mx-auto">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-3 flex-wrap mb-2">
            ...
            <span className="text-xs text-surface-400 font-mono">
              ...
              {pendingVersion && (
                <span className="ml-2 text-warning-dark font-sans">(v{pendingVersion.versionNumber} pending)</span>
              )}
            </span>
            <span className="text-xs text-surface-400">Updated {formatRelative(project.updatedAt)}</span>
          </div>
      ...
      {/* Approval bar — shown when a pending version exists */}
      {pendingVersion && diff && (
        <div className="mb-6">
          <VersionApprovalBar
      ...
      {`hiiii` + JSON.stringify(pendingVersion && activeVersion)}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card padding="md">
            ...
            {activeVersion?.descriptionJson ? (
              <RichTextViewer content={activeVersion.descriptionJson as Record<string, unknown>} />
            ) : (
            ...
            <Card padding="none">
              ...
              <div className="divide-y divide-surface-100">
                {allVersions.map((version) => {
                  ...
                  return (
                    <div key={version.id} className="flex items-center gap-4 px-6 py-3">
                      ...
                      <span className="text-xs text-surface-400">Phase {version.phaseNumber}</span>
                      <div className="flex-1" />
                      {version.signoffs.length > 0 && (
                        <div className="flex items-center gap-1">
                          {version.signoffs.map((s) => (
                            <Avatar key={s.id} name={s.user.name} size="xs" className="ring-2 ring-white" />
                          ))}
                        </div>
                      )}
              ...
              <div className="space-y-2">
                {project.phases.map((phase) => (
                  <div key={phase.id} className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${phase.status === "ACTIVE" ? "bg-brand-500" : phase.status === "COMPLETE" ? "bg-success" : "bg-surface-200"}`} />
                    <span className="text-sm text-surface-700 flex-1">
                      Phase {phase.phaseNumber}{phase.title ? ` — ${phase.title}` : ""}
                    </span>
                    <span className={`text-xs ${phase.status === "ACTIVE" ? "text-brand-600 font-medium" : phase.status === "COMPLETE" ? "text-success" : "text-surface-400"}`}>
