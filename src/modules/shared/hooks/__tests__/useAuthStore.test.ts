...
describe("useAuthStore", () => {
  ...
  it("setUser stores a user", () => {
    const mockUser = { id: "1", name: "Alice", email: "a@b.com", role: "CONSULTANT" as const };
    useAuthStore.getState().setUser(mockUser);
    expect(useAuthStore.getState().user).toEqual(mockUser);
  });

  it("setUser(null) clears the user", () => {
    useAuthStore.getState().setUser({ id: "1", name: "A", email: "a@b.com", role: "LEARNER" as const });
    useAuthStore.getState().setUser(null);
    expect(useAuthStore.getState().user).toBeNull();
  });

  it("isConsultant returns true for CONSULTANT role", () => {
    useAuthStore.getState().setUser({ id: "1", name: "A", email: "a@b.com", role: "CONSULTANT" });
    expect(useAuthStore.getState().isConsultant()).toBe(true);
  });

  it("isLearner returns true for LEARNER role", () => {
    useAuthStore.getState().setUser({ id: "1", name: "A", email: "a@b.com", role: "LEARNER" });
    expect(useAuthStore.getState().isLearner()).toBe(true);
  });

  it("isAdmin returns true for ADMIN role", () => {
    useAuthStore.getState().setUser({ id: "1", name: "A", email: "a@b.com", role: "ADMIN" });
